...
export const companies: Company[] = expandSeed(companyNames, 50, "growth-company").map(
...
export const roles: Role[] = expandSeed(roleNames, 50, "job-role").map(({ slug, name, category }, index) => ({
  id: `role-${index + 1}`,
  slug,
  title: name,
...
export const locations: Location[] = locationNames.map(([slug, name, type, state], index) => ({
  id: `location-${index + 1}`,
  slug,
  name,
...
export const industries: Industry[] = industryNames.map(([slug, name], index) => ({
  id: `industry-${index + 1}`,
  slug,
  name
}))
...
export function companyRecord(company: Company, index: number): EntityRecord {
...
export function roleRecord(role: Role, index: number): EntityRecord {
...
export function locationRecord(location: Location, index: number): EntityRecord {
...
export function industryRecord(industry: Industry, index: number): EntityRecord {
...
export const companyRecords = companies.map(companyRecord)
export const roleRecords = roles.map(roleRecord)
export const locationRecords = locations.map(locationRecord)
export const industryRecords = industries.map(industryRecord)
